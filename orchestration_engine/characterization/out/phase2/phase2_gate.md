...
**Status:** IN PROGRESS (2 checklist items pending)

## Phase 2 crossover (measured scatter + full-path delivery)

**Verdict:** `FULL_PATH_ADVANTAGE`

Streaming cosim: 16.2 cycles/completion steady-state (0.0401 us) + PCIe delivery -> ~3.9x vs kernel-bypass.

- Hardware scatter (fan-out=2): **16.2 cycles** = **0.0401 us** @ 404.4 MHz (streaming cosim (steady-state cycles/completion))
- Cosim one-shot latency: **59 cycles** (ap_start/ap_done)
- Cosim measured II: **47 cycles** (multi-transaction steady-state)
...
|----------|---------------|----------------|------------------|
| LangGraph (deployed) | 1334.83 | 1689.4x | 9527.7x |
| asyncio + epoll (measured full-path) | 5.37 | 6.8x | 38.3x |
| asyncio + kernel-bypass (measured full-path) | 3.12 | 3.9x | 22.3x |
...
| HLS cosim scatter (trust cycle count) | done | oe_scatter_cosim_proj/sol1/sim/report/oe_hls_scatter_kernel_cosim.rpt (59 cycles) |
| Streaming scatter cosim (steady-state cycles/completion) | done | C:\Users\zjohn\Projects\gnn-hls-accel\orchestration_engine\characterization\out\phase2\cosim_stream.json |

# Streaming scatter csynth + cosim (steady-state II). From repo root:
#   vitis_hls -f orchestration_engine/run_hls_scatter_stream.tcl
#
# One invocation processes 8 completions back-to-back through axis streams;
# cosim latency / 8 = measured steady-state cycles per completion.

open_project -reset oe_stream_proj
set_top oe_hls_scatter_stream

add_files orchestration_engine/hls/orchestration_engine.cpp -cflags "-I./orchestration_engine/hls"
add_files -tb orchestration_engine/tb/oe_hls_stream_tb.cpp -cflags "-I./orchestration_engine/hls"

open_solution -reset sol1 -flow_target vivado
set_part {xczu3eg-sbva484-1-e}
create_clock -period 3.33 -name default

csim_design
csynth_design

config_cosim -trace_level none
cosim_design

exit
